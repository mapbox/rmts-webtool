# Mapbox Fork of gdal3.js
This repository is maintained by the Solutions Architecture team at Mapbox. The code is used in the [RMTS Web Tool](https://labs.mapbox.com/rmts-tool/). A fork of the [gdal3.js](https://github.com/bugra9/gdal3.js) library was created to expose more GDAL methods and ensure functionality with a wider range of raster files.

## Mapbox Install Notes
These notes reference issues encountered when building the library on a 2020 MacBook Pro (2.3 GHz Quad-Core Intel Core i7)
- Compilation failed when building the webp library. In `build/native/src/libwebp-1.3.2/examples/vwebp.c`, `<GL/freeglut.h>` had to be changed to `<GLUT/freeglut.h>`
- In the Makefile for zlib, `AR=libtool` had to be changed to `AR=emar` and `ARFLAGS` had to be set to `ARFLAGS=rcs`

---
## All text below is from the source repository's README when it was forked at version 2.8.0.
---
